...
<form method="post" autocomplete="off">
  ...
  <div class="input-group no-gap-all">
    <%-- Title block --%>
    <c:if test="${!empty title}">
      <span class="align-self-middle margin-right-20"><c:out value="${title}"/></span>
    </c:if>
    <%-- Categories block --%>
    <c:if test="${showCategories eq 'true' && !empty categoryList}">
      <select name="categoryId${widgetContext.uniqueId}">
        <option value="-1">All <c:out value="${collection.name}" /></option>
    ...
    <%-- Input field blocks --%>
    <input type="search" placeholder="Search by keyword..." id="name" name="name" value="<c:if test="${!empty searchName}"><c:out value="${searchName}" /></c:if>" autocomplete="off">
