  ...
  <div class="grid-x grid-margin-x">
    <div class="medium-4 cell">
      ...
      <c:forEach items="${menuTabList}" var="menuTab" varStatus="tabStatus">
        <a href="#" class="js-mySubmit u-td-underline" data-src="${ctx}${menuTab.link}" data-target-id="<c:out value="${inputId}"/>" data-target-attr="href" title="<c:out value="${menuTab.name}" />">${ctx}<c:out value="${menuTab.link}" /></a>
        <c:forEach items="${menuTab.menuItemList}" var="menuItem" varStatus="itemStatus">
          <br />
          <a href="#" class="js-mySubmit u-td-underline" data-src="${ctx}${menuItem.link}" data-target-id="<c:out value="${inputId}"/>" data-target-attr="href" title="<c:out value="${menuItem.name}" />">${ctx}<c:out value="${menuItem.link}" /></a>
        </c:forEach>
    ...
    <div class="medium-6 cell">
      ...
      <c:forEach items="${fileItemList}" var="file" varStatus="status">
        ...
        <a href="#" class="js-mySubmit u-td-underline" data-src="${ctx}/assets/view/${file.url}" data-target-id="<c:out value="${inputId}"/>" data-target-attr="href"><c:out value="${file.title}" /></a>
