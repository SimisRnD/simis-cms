...
<jsp:useBean id="useSmallHighlight" class="java.lang.String" scope="request"/>
  ...
  <c:forEach items="${masterMenuTabList}" var="menuTab" varStatus="menuTabStatus">
    <c:choose>
      ...
      <c:when test="${!empty menuTab.menuItemList}">
        ...
        <c:if test="${useSmallHighlight eq 'true'}">
      ...
      <c:otherwise>
        <li<c:if test="${useSmallHighlight eq 'true' && fn:startsWith(pagePath, menuTab.link)}"> class="active"</c:if>><a href="${ctx}${menuTab.link}"><c:out value="${menuTab.name}" /></a></li>
      </c:otherwise>
    </c:choose>
  </c:forEach>
</ul>