...
<div class="platform-content-container">
  ...
  <c:forEach items="${contentTabList}" var="contentTab" varStatus="tabStatus">
    <li class="tabs-title<c:if test="${contentTab.isActive}"> is-active</c:if>"><a href="${ctx}<c:out value="${contentTab.link}" />"<c:if test="${contentTab.isActive}"> aria-selected="true"</c:if>><c:out value="${contentTab.name}" /></a></li>
  </c:forEach>
  </ul>
