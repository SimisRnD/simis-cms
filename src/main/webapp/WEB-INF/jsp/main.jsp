...
<c:choose>
  ...
  <c:if test="${!empty themePropertyMap}">
      <style><%-- Prevent top-bar flicker --%>
        ...
        <c:if test="${!empty themePropertyMap['theme.button.primary.hoverBackgroundColor']}">.button.primary:hover:not(.clear):not(.hollow), .button.primary:focus:not(.clear):not(.hollow), #platform-menu ul.menu li a.button.primary:hover:not(.clear):not(.hollow){background-color:var(--sc-button-primary-hover-background-color)}</c:if>
        ...
        <c:if test="${!empty themePropertyMap['theme.button.secondary.hoverBackgroundColor']}">.button.secondary:hover:not(.clear):not(.hollow), .button.secondary:focus:not(.clear):not(.hollow), #platform-menu ul.menu li a.button.secondary:hover:not(.clear):not(.hollow){background-color:var(--sc-button-secondary-hover-background-color)}</c:if>
        ...
        <div class="off-canvas position-left reveal-for-medium admin-menu hide-for-print" style="z-index: 1005 !important; padding-bottom: 50px" id="offCanvas" data-off-canvas>
          ...
          <c:if test="${userSession.hasRole('admin') || userSession.hasRole('content-manager')}">
            <ul class="vertical menu">
              ...
              <li<c:if test="${fn:startsWith(pageRenderInfo.name, '/admin/page-templates')}"> class="is-active"</c:if>><a href="${ctx}/admin/page-templates"><i class="${font:far()} fa-swatchbook fa-fw"></i> <span>Page Templates</span></a></li>
    ...
    <script nonce="${cspNonce}">
      ...
      $(document).ready(function() {
        ...
        <%-- CSP-safe admin action links (issue #1188). PageServlet sends Content-Security-Policy
             ...
             silently did nothing. Such links now declare data-confirm-post (the confirm message) and
             data-post-url (the POST target) and are bound here instead. confirmPostAction/postAction are
             the same globals those inline handlers called, defined further down in this nonce'd block. --%>
