...
<c:choose>
  <c:when test="${showPrice eq 'true' && !empty productSku.price && productSku.price gt 0}">
    <input type="submit" class="button expanded secondary" value="<c:out value="${comingSoonText}" /> &bull; <fmt:formatNumber type="currency" currencyCode="USD" value="${productSku.price}" />"/>
  </c:when>
  <c:otherwise>
    <input type="submit" class="button expanded secondary" value="<c:out value="${comingSoonText}" />"/>
  </c:otherwise>
</c:choose>