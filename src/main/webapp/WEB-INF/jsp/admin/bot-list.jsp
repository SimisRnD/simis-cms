...
<p class="help-text page-help">Upload requires a "Partial User Agent" column, plus optional "Label" and "Remove" columns; set Remove to "true" on a row to remove that entry instead of adding it. Download includes Partial User Agent, Label, and Date. After any bulk import, check the actual result summary rather than assuming every row landed.</p>
  ...
  <tbody>
    ...
    <tr>
      <td nowrap="true">
        ...
        <a aria-label="Remove ${fn:escapeXml(record.userAgent)} from the bot list" href="#" data-confirm-post="Are you sure you want to remove <c:out value="${record.userAgent}" /> from the bot list?" data-post-url="${widgetContext.uri}?command=delete&widget=${widgetContext.uniqueId}&token=${userSession.formToken}&botListId=${record.id}"><i aria-hidden="true" class="fa fa-remove"></i></a>
      </td>
