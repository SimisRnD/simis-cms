...
<form method="post">
  ...
  <c:if test="${prefix eq 'security'}">
    ...
    <p class="help-text">The password policy below applies the moment it's saved, but only to a password being set from that point on -- it is never checked retroactively against a password someone already has, since there's no way to test length/complexity against an existing hash without knowing the plaintext. Someone with an older, shorter password keeps using it until they set a new one (their own reset, an admin-forced reset, or self-registration).</p>
  </c:if>
