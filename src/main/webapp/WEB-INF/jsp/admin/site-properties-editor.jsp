      ...
      <tr>
        ...
        <td nowrap>
          ...
          <c:if test="${siteProperty.name eq 'mail.ssl'}">
            <p class="help-text" id="mailSslHelpText">When on, SimIS connects to the SMTP server above using implicit SSL/TLS -- the connection is encrypted from the moment it opens, traditionally on port 465. This and the STARTTLS toggle below are alternatives, not layers: a server offers one or the other on a given port, so turn on whichever one your provider documents for the port you entered above, and never both. (If both are on, SimIS uses implicit SSL, ignores STARTTLS, and logs a warning.) With both off the connection is unencrypted and the SMTP password below is sent in clear text, which is only appropriate for a local or internal relay. Per each provider's own documentation, Gmail's smtp.gmail.com offers both port 465 with this toggle on and port 587 with STARTTLS, so either works.</p>
          </c:if>
          <c:if test="${siteProperty.name eq 'mail.starttls'}">
            <p class="help-text" id="mailStartTlsHelpText">When on, SimIS opens a plain connection to the SMTP server above and immediately upgrades it to TLS -- the STARTTLS scheme, traditionally on port 587. This is what most hosted providers use, and several offer no implicit-SSL alternative: <a href="https://learn.microsoft.com/en-us/exchange/mail-flow-best-practices/how-to-set-up-a-multifunction-device-or-application-to-send-email-using-microsoft-365-or-office-365" target="_blank" rel="noreferrer">Microsoft 365</a> documents that its client SMTP submission does not support port 465 at all, and Azure Communication Services is likewise STARTTLS-only. Encryption here is required rather than merely attempted: if the server does not offer STARTTLS the send fails instead of quietly continuing unencrypted, since falling back would put the SMTP password below on the wire in clear text. Use this or the SSL toggle above, whichever your provider documents -- not both.</p>
          </c:if>
