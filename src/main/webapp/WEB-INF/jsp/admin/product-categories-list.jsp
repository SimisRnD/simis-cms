...
<%--        <input type="text" name="displayOrder${status.count}" value="${status.count}" />--%>
      ...
      <td>
        <a aria-label="Edit ${fn:escapeXml(productCategory.name)}" href="${ctx}/admin/product-category?productCategoryId=${productCategory.id}&returnPage=/admin/product-categories"><i aria-hidden="true" class="fa fa-edit"></i></a>
        <a aria-label="Delete ${fn:escapeXml(productCategory.name)}" href="#" data-confirm-post="Are you sure you want to delete <c:out value="${productCategory.name}" />?" data-post-url="${widgetContext.uri}?command=delete&widget=${widgetContext.uniqueId}&token=${userSession.formToken}&productCategoryId=${productCategory.id}"><i aria-hidden="true" class="fa fa-remove"></i></a>
      </td>
    </tr>
    </c:forEach>
