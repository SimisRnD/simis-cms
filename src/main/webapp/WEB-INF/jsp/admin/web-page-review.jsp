...
<c:if test="${!empty webPage}">
  ...
  <c:choose>
    <c:when test="${reviewOffer eq 'publish'}">
      <a class="button warning" href="${widgetContext.uri}?action=publish&webPageId=${webPage.id}&widget=${widgetContext.uniqueId}&token=${userSession.formToken}" data-confirm-href="Publish this page?">PUBLISH</a>
    </c:when>
    <c:when test="${reviewOffer eq 'submit'}">
      <a class="button warning" href="${widgetContext.uri}?action=submitForReview&webPageId=${webPage.id}&widget=${widgetContext.uniqueId}&token=${userSession.formToken}" data-confirm-href="Submit this page for review?">SUBMIT FOR REVIEW</a>
    </c:when>
    ...
    <c:when test="${reviewOffer eq 'decide'}">
      ...
      <form method="post" action="${widgetContext.uri}" class="platform-content-review-form" data-confirm-submit="Approve and publish this page?">
      ...
      <a class="button alert" href="${widgetContext.uri}?action=reject&webPageId=${webPage.id}&widget=${widgetContext.uniqueId}&token=${userSession.formToken}" data-confirm-href="Return this page to the author?">REJECT</a>
    </c:when>
    <c:otherwise>
