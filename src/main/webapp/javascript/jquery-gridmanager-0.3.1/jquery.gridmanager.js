...
(function ($) {
  ...
  $.gridmanager = function (el, options) {
    ...
    gm.initDefaultButtons = function () {
      ...
      if (gm.options.editableRegionEnabled) {
        gm.options.customControls.global_col.push({
          callback: gm.addEditableAreaClick,
          loc: 'top',
          iconClass: 'fa fa-plus',
          title: 'Add Content Widget'
        });
      }
    };
    ...
    gm.createControls = function () {
      ...
      $.each(gm.options.controlButtons, function (i, val) {
        ...
        var _text = _class.substr(1);
        buttons.push("<a title='Add Row " + _class + "' class='" + gm.options.controlButtonClass + " add" + _class + "'><span class='" + gm.options.controlButtonSpanClass + "'></span> " + _text + "</a>");
    ...
    gm.switchLayoutMode = function (mode) {
      var canvas = gm.$el.find("#" + gm.options.canvasId), temp_html = canvas.html(), regex1 = '', regex2 = '',
        uimode = '';
      ...
      switch (mode) {
        case 768:
          ...
          $(uimode).attr('class', 'far fa-fw fa-tablet').attr('title', 'Tablet');
          break;
        case 640:
          regex1 = '(' + gm.options.colDesktopClass + '\\d+)';
          ...
          $(uimode).attr('class', 'far fa-fw fa-mobile').attr('title', 'Phone');
          break;
        default:
          ...
          $(uimode).attr('class', 'far fa-fw fa-desktop').attr('title', 'Desktop');
      }
    ...
    gm.initControls = function () {
      ...
      }).on("click", "button.gm-save", function () {
        gm.deinitCanvas();
        gm.saveremote();
      ...
      }).on("click", "a.gm-resetgrid, a.gm-remove, a.gm-removeRow, button.gm-save, button.gm-preview, a.gm-viewsource, a.gm-addColumn, a.gm-colDecrease, a.gm-colIncrease", function (e) {
    ...
    gm.saveremote = function () {
      ...
      $.ajax({
        ...
        if (data[0].message) {
          ...
          window.location.href = gm.options.redirectURL;
        }
      });
    ...
    gm.createRow = function (colWidths) {
      var row = $("<div/>", {"class": gm.options.rowClass + " margin-bottom-30 " + gm.options.gmEditClass});
    ...
    gm.addEditableAreaClick = function (container, btn) {
      ...
      $(('.' + gm.options.gmToolClass + ':last'), container)
        .before(elem = $('<div>').addClass(gm.options.gmEditRegion + ' ' + gm.options.contentDraggableClass)
          .append(gm.options.controlContentElem + '<div class="' + gm.options.gmContentRegion + '"><p>Write your content</p></div>')).before(cTagClose).prev().before(cTagOpen);
    ...
    gm.editableAreaFilter = function (canvasElem, isInit) {
      if (isInit) {
        var cTagOpen = '<!--' + gm.options.gmEditRegion + '-->',
          ...
          rep =
            cTagOpen +
            '<div class="' + gm.options.gmEditRegion + ' ' + gm.options.contentDraggableClass + '">' +
            gm.options.controlContentElem + '<div class="' + gm.options.gmContentRegion + '">$1</div></div>' + cTagClose;
   ...
   */
    ...
    */
    ...
    // Adds any missing classes in columns for multi-device support.
