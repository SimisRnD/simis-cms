 ...
 */
public class SendEcommerceManagerEmailCommand {

  private static Log LOG = LogFactory.getLog(SendEcommerceManagerEmailCommand.class);

  public static void sendMessage(String subject, String html, String text) {

    // Send to all Community Managers
    Role role = RoleRepository.findByCode("ecommerce-manager");
    if (role == null) {
      return;
    }
    ...
    try {
      ...
      for (User user : userList) {
        if (user.getEmail().contains("@")) {
          LOG.debug("Sending ecommerce-manager email to: " + user.getEmail() + " " + user.getFullName());
